...
func (receiver DingTalkError) Error() string {
	return receiver.Msg
}
...
func (dtr DingTalkRobot) SendMsg(msg DingTalkMsg) error {
	...
	if err != nil {
		return DingTalkError{err.Error()}
	}
	return nil
}
...
func (dtc *DingTalkClient) GetAccessToken() (*dingTalkAccessTokenResp, error) {
	...
	if err != nil {
		return nil, DingTalkError{"json解析钉钉接口响应错误"}
	}
	if r.ErrCode == 0 {
		c.Update(dtc.AppKey, r.AccessToken)
		return &r, nil
	} else {
		return nil, DingTalkError{r.ErrMsg}
	}
}
